...
def test_many_vehicles_register(default_environment_fixture):
    default_environment_fixture.register_vehicle(
        Vehicle('tp1', *default_environment_fixture.get_random_coordinates(), vehicle_type=VT.Normal)
    )
    default_environment_fixture.register_vehicle(
        Vehicle('tp2', *default_environment_fixture.get_random_coordinates(), vehicle_type=VT.Normal)
    )
...
def test_get_closest_emergency_vehicle(environment_with_vehicles):
    ...
    assert environment_with_vehicles.get_closest_emergency_vehicle(vehicle.x, vehicle.y).id == 'ev2'
    assert environment_with_vehicles.get_closest_emergency_vehicle(vehicle2.x, vehicle2.y).id == 'ev1'
